...
export default function ShareLinkBox({ shareUrl }: ShareLinkBoxProps) {
  ...
  // ✅ Safe: window is only used in the browser after mount
  useEffect(() => {
  ...
  if (!sharelink) return null; // avoid rendering before it's ready
  ...
  return (
    <div className="max-w-7xl mx-auto bg-gray-900/60 border border-gray-800 rounded-xl p-4 flex items-center justify-between text-sm">
      ...
      <button
        onClick={handleCopy}
        className={`w-[80px] cursor-pointer ml-3 px-3 py-2 rounded-md font-semibold transition ${
          copied
